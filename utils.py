...
class Agent:
    ...
    def chat(self, message, img_path=None, chat_mode=True):
        # Before sending message to the model, ensure it is more medical if it's agent communication
        # Refine only if not already refined
        if message.strip()[:9] == "[REFINED]":
            message = refine_agent_message_medically(message, self.model_info)
    ...
    def temp_responses(self, message, img_path=None):
        # Also refine message here if needed
        message = refine_agent_message_medically(message, self.model_info)
        self.messages.append({"role": "user", "content": message})
...
class Group:
    def __init__(self, goal, members, question, examplers=None):
        ...
        for member_info in members:
            # Ensure roles and instructions also get refined if needed
            instr = f'You are a {member_info["role"]} who {member_info["expertise_description"].lower()}.'
            instr = refine_agent_message_medically(instr, 'gpt-4o-mini')
            _agent = Agent(instr, role=member_info['role'], model_info='gpt-4o-mini')
            _agent.chat(instr)
...
You have the following assistant clinicians working with you:'''
            for a_mem in assist_members:
                delivery_prompt += f"\n{a_mem.role}"

            delivery_prompt += f"\n\nGiven the medical query, provide a short answer on what investigations are needed from each assistant clinician.\nQuestion: {self.question}"

            delivery_prompt = refine_agent_message_medically(delivery_prompt, 'gpt-4o-mini')
            if lead_member:
                ...
                if assist_members:
                    ...
                    delivery = "No agents available."

            investigations = []
            for a_mem in assist_members:
                sub_prompt = f"You are in a medical group where the goal is to {self.goal}. Your group lead requests the following investigations:\n{delivery}\n\nRemind your expertise and return your investigation summary with core info."
                sub_prompt = refine_agent_message_medically(sub_prompt, 'gpt-4o-mini')
                investigation = a_mem.chat(sub_prompt)
            ...
            if self.examplers is not None:
                investigation_prompt = f"""The gathered investigation from your assistant clinicians is:
{gathered_investigation}

After reviewing the following example cases:
{self.examplers}

Question: {self.question}

Return your answer to the medical query among the options provided."""
            else:
                investigation_prompt = f"""The gathered investigation from your assistant clinicians is:
{gathered_investigation}

Now, return your answer to the medical query among the options provided.

Question: {self.question}"""

            investigation_prompt = refine_agent_message_medically(investigation_prompt, 'gpt-4o-mini')
            if lead_member:
                response = lead_member.chat(investigation_prompt)
            else:
                response = "No lead member found."
            return response
...
def setup_model(model_name):
    # We're no longer using gemini-pro or external frameworks, just GPT-4o
...
def determine_difficulty(question, difficulty, model):
    ...
    difficulty_prompt = f"""You are a medical reasoning assistant. Given the medical query:\n{question}\n\nDecide the complexity:
1) basic: single agent
2) intermediate: multiple experts
3) advanced: multiple teams"""

    medical_agent = Agent(instruction='You are a medical expert deciding complexity.', role='medical expert',
                          model_info=model)

    medical_agent.chat('You are a medical expert deciding complexity.')
...
def process_basic_query(question, examplers, model, args):
    ...
    if args.dataset == 'medqa':
        ...
        for ie, exampler in enumerate(examplers[:5]):
            ...
            exampler_reason = medical_agent.chat(
                f"You are a helpful medical agent. Here is an example question and answer. Provide 1-2 sentences of reasoning:\nQuestion: {exampler_question}\nAnswer: {exampler_answer}")
    ...
    single_agent = Agent(instruction='You are a helpful assistant for medical MCQs.', role='medical expert',
                         examplers=new_examplers, model_info=model)
    single_agent.chat('You are a helpful assistant for medical MCQs.')
    final_decision = single_agent.temp_responses(
        f'''Consider the following medical question. Let's think step by step.\n\n**Question:** {question}\nAnswer: ''',
        img_path=None)
...
def process_intermediate_query(question, examplers, model, args):
    ...
    agent_emoji = ['\U0001F468\u200D\u2695\uFE0F', '\U0001F468\U0001F3FB\u200D\u2695\uFE0F',
    ...
    interaction_log = {f'Round {round_num}': {f'Turn {turn_num}': {
        f'Agent {source_agent_num}': {f'Agent {target_agent_num}': None for target_agent_num in
                                      range(1, num_agents + 1)} for source_agent_num in range(1, num_agents + 1)} for
                                              turn_num in range(1, num_turns + 1)} for round_num in
    ...
    for n in range(1, num_rounds + 1):
        ...
        agent_rs = Agent(
            ...
            role="medical assistant", model_info=model)
        agent_rs.chat(
        ...
        for turn_num in range(num_turns):
            ...
            for idx, v in enumerate(medical_agents):
                all_comments = "".join(f"{_k} -> Agent {idx + 1}: {_v[f'Agent {idx + 1}']}\n" for _k, _v in
                                       interaction_log[round_name][turn_name].items())
                ...
                if 'yes' in participate.lower().strip():
                    ...
                    chosen_experts = [int(ce) for ce in chosen_expert.replace('.', ',').split(',') if
                                      ce.strip().isdigit()]
...
#########################################
# Additional Helper Functions for Refinement
#########################################

def refine_question_medically(question, model_info):
    # Use GPT-4o to rewrite the question in more detailed, medically appropriate terms
    instruction = (
        "You are a medical specialist. Given the below medical question, rewrite it in a more medically detailed "
        "and professional manner, using correct medical terminology and clarifying any ambiguous aspects.")
    agent = Agent(instruction=instruction, role='professor', model_info=model_info)
    refined_question = agent.chat(f"Original Question:\n{question}\n\nRewrite it:")
    return refined_question


def refine_agent_message_medically(message, model_info):
    # Use GPT-4o to ensure the message is medically appropriate and uses standard terminology.
    # If message is too short or non-medical, just return as is.
    if message.strip()[:9] == "[REFINED]":
        return message
    if not message.strip():
        return message
    instruction = ("You are a medical language specialist. Take the following text and rewrite it using correct, "
                   "professional medical terminology. Ensure clarity, precision, and adherence to medical standards. "
                   "Do not change the meaning, only improve the medical quality of the language.")
    temp_agent = Agent(instruction=instruction, role='cross-communicator', model_info=model_info)
    refined_msg = temp_agent.chat(f"Original:\n{message}\n\nRewrite medically:")
    return refined_msg
